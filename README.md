...
Pipeline batch que cruza predicoes vs candles reais, segmenta por contexto,
detecta drift e auto-executa hipoteses declaradas em `vault/Hypotheses/`.
...
# Wrapper agendavel (preferido — entry point estavel em scripts/)
python scripts/run_daily_eval.py
python scripts/run_daily_eval.py --date 2026-04-24
python scripts/run_daily_eval.py --from 2026-04-24 --to 2026-04-30

# Equivalente direto via modulo
python -m src.evaluation.daily_eval [--date | --from --to]
```
...
python scripts/scheduler.py --run-once eval  # roda 1x e sai
python scripts/scheduler.py                  # foreground
```

> **Producao:** `schedule` eh in-process — nao sobrevive reboot. Para SLA real,
> use Windows Task Scheduler chamando `run_daily_eval.py` diretamente, ou rode
> `scheduler.py` via NSSM como servico Windows.

**Idempotente.** Pode rodar quantas vezes quiser na mesma data — sobrescreve.

### O que ele faz

1. **Carrega predicoes** com schema enriquecido (`model_version` not null) do dia.
2. **Cruza com candles** reais em t+5/10/15min (M5: t+1, t+2, t+3).
3. **Arquiva candles** localmente em `data/archive/candles/symbol=X/date=Y/`
   (UNION+dedup) — protege contra rotacao do buffer `data/raw/`.
4. **Agrega metricas** por model, symbol, session, regime, confidence bin, signal.
5. **Drift detection**: compara hit rate por modelo hoje vs media rolling 30d.
   Flag > 10pp.
6. **Auto-executa hipoteses** com `filters` no YAML frontmatter de
   `vault/Hypotheses/*.md`. Atribui verdict (UNDERPOWERED / REJECTED_WR / WEAK /
   PROMISING) e append em "Daily eval log" no fim da nota.
7. **Gera relatorio markdown** em `vault/Research/eval-daily/{date}.md`.
8. **Salva dataset** em `data/research/eval_{date}.parquet` para reuso.

### Cadencia recomendada

**Diaria, end-of-day UTC.** Rodar imediatamente apos `upload_to_s3.py` se ambos
forem usados. Suporta gaps — se ficar sem rodar 7 dias, basta rodar com `--from
--to` no fim, **desde que o uploader (ou o proprio eval) tenha arquivado os
candles antes de rotacionarem do buffer**.

### Vault Obsidian (`vault/`)

Vault versionado no repo com research, hipoteses, backtests, demo trading,
postmortems e ideas. Templates pre-prontos em `vault/Templates/`. Ver
`vault/README.md` para fluxo de uso e regras de higiene (hipotese ANTES do
teste, link entre notas, sem journal emocional no git).

### Hipoteses com auto-eval

Adicione `filters` no YAML frontmatter de qualquer hipotese para rodar em todo
daily eval:
...
Suportado: qualquer coluna do `eval_{date}.parquet` (`confidence`, `symbol`,
`model`, `session`, `regime_trend`, `regime_vol`, `regime_range`, `signal`).
Validacao formal continua manual via `conditional_analysis.evaluate_filter`
(holdout protegido, Bonferroni cumulativo).
...
- **Mudancas recentes:** [`.project/CHANGELOG.md`](.project/CHANGELOG.md)
- **Historico legado (2026-04-07 a 2026-04-13):** [`.project/CHANGELOG_LEGACY.md`](.project/CHANGELOG_LEGACY.md)
- **Decisoes arquiteturais:** [`.project/DECISIONS.md`](.project/DECISIONS.md)
- **Vault de research/hipoteses:** [`vault/README.md`](vault/README.md)
